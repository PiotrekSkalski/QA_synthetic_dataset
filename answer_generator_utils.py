...
def dynamic_padding_collate_fn(batch_list):
    """
    A collate function used in training the answer generator that works with
    examples from the SQuAD2.0 dataset that have preprocessed with 'preprocess_dataset'
    function.
    """
...
def preprocess_dataset(dataset):
    """
    Preprocesses the dataset returned by 'load_and_cache_examples' function from
    data_processor.py for the purpose of training the answer generator model, i.e
    it extracts the context and the start and end token indices.
    """
...
def generator_collate_fn(batch, tokenizer, max_context_length):
    """
    A collate function used in the answer generator together with contexts
    sampled from WikiDataset.
    """
...
class GeneratorBatchSampler(Sampler):
    """
    A batch sampler used in the answer generator that works with WikiDataset.
    It makes sure that sampled passages of text have a specified minimum length
    after tokenization.
    """
